<#
.SYNOPSIS
    Build Cove GIF Maker into a Windows Setup installer and a single-file
    portable executable. Both artifacts bundle ffmpeg, ffprobe, and gifsicle.
...
.DESCRIPTION
    ...
    release-essentials build and the eternallybored.org gifsicle build, then
    runs PyInstaller twice:
...
# Invoke-WebRequest in PowerShell 7 buffers the full response into a single
# MemoryStream before flushing to disk, which throws "Stream was too long" on
# some servers. curl.exe ships with Windows 10+ and handles this cleanly.
...
Step "[4/8] Downloading gifsicle"
$gsTmp = Join-Path ([IO.Path]::GetTempPath()) ("gifsicle-" + [Guid]::NewGuid())
New-Item -ItemType Directory -Path $gsTmp | Out-Null
$gsZip = Join-Path $gsTmp "gifsicle.zip"
Download-File $GifsicleUrl $gsZip
Expand-Archive -Path $gsZip -DestinationPath $gsTmp -Force
$gifsicleExe = Get-ChildItem -Path $gsTmp -Filter gifsicle.exe -Recurse |
               Select-Object -First 1 -ExpandProperty FullName
if (-not $gifsicleExe) { throw "gifsicle.exe missing from downloaded archive" }
...
$commonArgs = @(
    ...
    '--icon', 'cove_icon.ico',
    '--paths', 'src',
    '--add-data', ("src\cove_gif_maker\assets\cove_icon.png" + [IO.Path]::PathSeparator + "cove_gif_maker\assets"),
    ...
    '--add-binary', ($gifsicleExe + [IO.Path]::PathSeparator + '.'),
...
# Drop the licenses alongside the installed app
...
# Locate Inno Setup's iscc.exe (pre-installed on windows-latest runners).
$iscc = $null
...
Remove-Item -Recurse -Force $ffTmp, $gsTmp -ErrorAction SilentlyContinue
