...
def sample_color_at(video: Path, at_time: float, x: int, y: int) -> str | None:
    ...
    Returns None on any sampling failure. This runs straight off a click
    in the preview, so a missing ffmpeg or an unseekable file has to come
    back as "no color" for the caller to report — the same way a failed
    auto-detect does — rather than raising through the Qt event handler."""
    try:
        return _sample_color_at(video, at_time, x, y)
    except Exception:  # noqa: BLE001
        return None


def _sample_color_at(video: Path, at_time: float, x: int, y: int) -> str | None:
