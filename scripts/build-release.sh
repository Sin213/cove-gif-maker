#!/usr/bin/env bash
# Build .AppImage and .deb for Cove GIF Maker.
#
# Requires:
#   - python3 (with pip)
#   - ar, tar, xz, curl
# Downloads a static ffmpeg+ffprobe build automatically. gifsicle is optional
# and resolved from PATH at runtime (listed as Recommends in the .deb).
#
...
APP_NAME="cove-gif-maker"
DISPLAY_NAME="Cove GIF Maker"
...
"$BUILD_ENV/bin/pyinstaller" \
    ...
    --name "$APP_NAME" \
    --paths src \
    --add-data "src/cove_gif_maker/assets/cove_icon.png:cove_gif_maker/assets" \
...
Type=Application
Name=$DISPLAY_NAME
GenericName=Video to GIF Converter
Comment=Trim videos and convert them to optimized GIFs
Exec=$APP_NAME
Icon=$APP_NAME
Terminal=false
Categories=AudioVideo;Video;Utility;
Keywords=gif;video;ffmpeg;trim;webp;convert;
StartupNotify=true
EOF
...
Section: video
Priority: optional
Homepage: https://github.com/Sin213/cove-gif-maker
Description: Trim videos and convert them to optimized GIFs or WebPs
 Cove GIF Maker is a focused desktop tool: drop a video, trim it on a visual
 timeline, crop it with a draggable rectangle, and convert it to an optimized
 GIF or WebP using ffmpeg's two-pass palette pipeline. ffmpeg is bundled;
 gifsicle is optional (used for a final optimization pass when available).
EOF
